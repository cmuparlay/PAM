...
/*
Q11_rtype Q11o(maps m, uint nation_id, float fraction) {
  timer t;
  t.start();    
  auto is_nation = [&] (supp_part_map2::E& se) -> bool {
    Supplier& s = se.second.first;
    return (s.nationkey == nation_id);
  };

  supp_part_map2 spm = supp_part_map2::filter(m.spm2, is_nation);
  t.next("filter");
  using kf_pair = pair<dkey_t,float>;
  using inner_t = supp_part_map2::V::second_type::E;

  auto get_value = [] (supp_part_map2::E& se, inner_t& p) -> kf_pair {
    Part_supp& ps = p.second.first.first;
    return kf_pair(ps.partkey, ps.availqty*ps.supplycost);
  };

  sequence<kf_pair> elts = flatten_pair<kf_pair>(spm, get_value);
  t.next("flatten");
  
  // sum revenue by custkey
  sequence<kf_pair> r = pbbs::collect_reduce_pair<Add<float>>(elts);
  size_t n = r.size();
  t.next("collect");
  
  auto second_f = [&] (size_t i) {return r[i].second;};
  float total = pbbs::reduce_add(make_sequence<float>(n, second_f));
  float cutoff = fraction*total;
  t.next("plus reduce");
      
  // only keep entries beyond a fraction of total
  // r is a sequence of <part,total_value> pairs
  sequence<bool> big_enough(n, [&] (size_t i) {
      return r[i].second >= cutoff;});
  sequence<kf_pair> r2 = pbbs::pack(r, big_enough);
  t.next("pack");
