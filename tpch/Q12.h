...
Q12_rtype Q12(maps m, char* mode1, char* mode2, char* start_date, char* end_date) {
  ...
  auto date_f = [&] (receipt_map::E& e) -> ret_t {
	...
	if (e.shipmode() == m1) return make_pair(make_pair(high, low), make_pair(0,0));
	if (e.shipmode() == m2) return make_pair(make_pair(0,0), make_pair(high, low));
      }
      return id;
    };
    li_map& lim = e.second;
