...
sequence<Q20rtype> Q20(maps m, bool verbose, const Date start, const Date end, dkey_t q_nation, const char* color) {		  
...
double Q20time(maps m, bool verbose) {
  ...
  char starts[] = "1994-01-01";
  char ends[] = "1994-12-31";
  const char color[] = "forest";
  dkey_t q_nation = 3;
  const Date start = Date(starts);
  const Date end = Date(ends);
  
  sequence<Q20rtype> res = Q20(m, verbose, start, end, q_nation, color);
