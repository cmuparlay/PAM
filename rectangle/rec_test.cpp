...
int main(int argc, char** argv) {
    ...
    if (argc != 7) {
        cout << argv[0] << " <n> <rounds> <num_queries> <dist> <win> <rsv>"<< std::endl;
        exit(1);
    }
	srand(2017);
	...
	dist = str_to_int(argv[4]);
	win = str_to_int(argv[5]);
	size_t query_num  = str_to_int(argv[3]);
	rsv = str_to_int(argv[6]);
