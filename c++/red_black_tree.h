...
struct red_black_tree {
  ...
  struct balance : Node {
    ...
    static bool is_balanced(node* t) {
	  ...
	  if (!((t->color == BLACK) ||
		(color(t->lc) == BLACK && color(t->rc) == BLACK))) {
			cout << "bad color" << endl;
			cout << color(t->lc) << " " << color(t) << " " << color(t->rc) << endl;
			f = false;
		}
  ...
  private:
    ...
    static node* right_join(node* t1, node* t2, node* k) {
		// if (!t1) {
			// if (!t2) cout << "null and null" << endl; else {
				// cout << "null and non-null" << endl;
				// cout << height(t1) << " " << height(t2) << endl;
			// }
		// }
      if (height(t1) == height(t2) && color(t1) == BLACK) 
		return balanced_join(t1, t2, k, RED);
	  //if (!t1) std::cout << "right_join fail" << std::endl;
    ...
    static node* left_join(node* t1, node* t2, node* k) {
		// if (!t2) {
			// if (!t1) cout << "null and null" << endl; else {
				// cout << "null and non-null" << endl;
				// cout << height(t1) << " " << height(t2) << endl;
			// }
		// }
      if (height(t1) == height(t2) && color(t2) == BLACK) 
		return balanced_join(t1, t2, k, RED);
	//if (!t2) std::cout << "left_join fail" << std::endl;
