...
using Q10_relt = tuple<dkey_t, char*, double, double, char*, char*, char*, char*>;
...
Q10_rtype Q10(maps m, const char* start, const char* end) {
  ...
  using kf_pair = pair<dkey_t, double>;
  ...
  auto date_f = [&] (order_map::E& e) -> kf_pair {
    ...
    double v = line_item_set::map_reduce(e.second.second, li_f, Add<double>());
    return make_pair(o.customer_key, v);
  };
  sequence<kf_pair> elts = flatten<kf_pair>(order_range, date_f);

  // sum revenue by custkey
  sequence<kf_pair> r = parlay::reduce_by_key(elts, parlay::plus<double>());
  //sequence<kf_pair> r = parlay::internal::group_by_and_combine(elts, parlay::addm<double>());
