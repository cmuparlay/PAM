...
void copy_string(char *dest, parlay::sequence<char> src, size_t max_len)
{
  ...
  parlay::parallel_for(0, src.size(), [&](size_t i)
                       { dest[i] = src[i]; });
  dest[src.size()] = 0;
}
...
struct Percent
{
  ...
  double val() const { return ((double)_val) * (double).01; }
...
struct lineitem_flags
{
  ...
  lineitem_flags(char linestatus, char returnflag, char shipinstruct,
                 char shipmode)
  {
    _flags = ((linestatus == 'O' ? 1 : 0) | (returnflag == 'A' ? 2 : 0) | (returnflag == 'N' ? 4 : 0) | (shipinstruct == 'T' ? 8 : 0) | (shipinstruct == 'D' ? 16 : 0) | (shipinstruct == 'C' ? 24 : 0) | (shipmode == 'A' ? 32 : 0) | (shipmode == 'T' ? 64 : 0) | (shipmode == 'F' ? 96 : 0) | (shipmode == 'G' ? 128 : 0) | (shipmode == 'M' ? 160 : 0) | (shipmode == 'S' ? 192 : 0));
  }
  char linestatus() const
  {
    return (_flags & 1) ? 'O' : 'F';
  }
  void set_linestatus() { _flags = _flags | 1; }
  char returnflag() const
  {
    return ((_flags & 6) == 0) ? 'R' : ((_flags & 6) == 2) ? 'A'
                                                           : 'N';
  }
  char shipinstruct() const
  {
    uchar t = _flags & 24;
    if (t == 0)
      return 'N';
...
struct LineItem
{
  ...
  size_t strings;
  LineItem() = default;
  LineItem(const Line &S)
  {
    // orderkey = str_to_int(parlay::make_slice(S[0].begin(), S[0].end()));
    check_length(S, 16);
    order_key = str_to_int(S[0]);
    part_key = str_to_int(S[1]);
    supplier_key = str_to_int(S[2]);
  ...
  quantity_t quantity() const { return strings & 255; }
...
struct Orders
{
  ...
  Orders(const Line &S)
  {
    check_length(S, 9);
    order_key = str_to_int(S[0]);
    customer_key = str_to_int(S[1]);
    status = S[2][0];
    total_price = str_to_double(S[3]);
    order_date = Date(S[4]);
    order_priority = S[5][0] - 48;
    shipment_priority = str_to_int(S[7]);
...
struct part_supp
{
  ...
  double supply_cost;
  part_supp(const Line &line)
  {
    check_length(line, 5);
    part_key = str_to_int(line[0]);
    supplier_key = str_to_int(line[1]);
    available_quantity = str_to_int(line[2]);
    supply_cost = str_to_double(line[3]);
  }
  part_supp() : part_key(0), supplier_key(0), available_quantity(0), supply_cost(0.0) {};
};
