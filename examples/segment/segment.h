...
void output_content(T t) {
  parlay::sequence<seg_type_2d> out;
...
struct SegmentQuery {
  ...
  void construct(parlay::sequence<seg_type_2d>& segs) {
    const size_t n = segs.size();
    reserve_tm.start();
    seg_tree::reserve(3*n);
    seg_set::reserve(50*n);
    reserve_tm.stop();
    auto end_points = parlay::sequence<seg_entry>::uninitialized(2*n);
    total_tm.start();
